#!/usr/bin/env python3
"""
BTC 선물 시그널 봇 v6 (B-Final) → v6.1
FVG + OB + 거래량 + HTF 필터 + 펀딩비 필터
+ 모멘텀 필터 + 켈리식 리스크 + 월간손실한도
데이터: OKX API (Railway 서버 접근 확인됨)

v6.1 변경사항 (수익 로직 변경 없음):
1. 보안: 텔레그램 토큰 → 환경변수
2. 상태 영속화: state.json (재시작 시 복원)
3. daily_loss / monthly_blocked 실제 동작
"""
...
def mark_signal_sent(direction, now_ts, expected_loss):
    ...
    if state["daily_loss"][date_key] >= TOTAL_SEED*DAILY_MAX_LOSS_PCT:
        ...
        print(f"  ⚠️ 일일손실 한도 도달: ${state['daily_loss'][date_key]:,.2f}")
    ...
    if monthly_total >= TOTAL_SEED*MONTHLY_MAX_LOSS_PCT:
        ...
        print(f"  ⚠️ 월간손실 한도 도달: ${monthly_total:,.2f}")
...
def run():
    print("="*60)
    print("  BTC 시그널 봇 v6.1 (B-Final + 상태영속화) - OKX API")
    ...
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        ...
        print("   Railway: Dashboard → Variables 탭에서 추가")
        return
